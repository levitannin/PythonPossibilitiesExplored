...
def old_multi_test():
    p1 = multiprocessing.Process(target = base_test)
    p2 = multiprocessing.Process(target = base_test)
...
def loop_multi_test():
    processes = []
    for _ in range(10):
        p = multiprocessing.Process(target = base_test)
        p.start()
        processes.append(p)
...
def loop_arg_multi_test(seconds):
    #   Must be able to serialize args using pickle
    processes = []
    for _ in range(10):
        p = multiprocessing.Process(target = base_arg_test, args = [seconds])
        p.start()
        processes.append(p)
...
def new_multi_test(seconds):
    with concurrent.futures.ProcessPoolExecutor() as executor:
        f1 = executor.submit(base_arg_test, seconds)
        print(f1.result())

def new_multi_test_loop(seconds):
    with concurrent.futures.ProcessPoolExecutor() as executor:
...
def new_multi_test_loop2(seconds):
    with concurrent.futures.ProcessPoolExecutor() as executor:
...
def new_multi_test_map(seconds):
    with concurrent.futures.ProcessPoolExecutor() as executor:
